...
def hardware_enabled():
    ...
    return (is_raspberry_pi() and 
            os.environ.get('HARDWARE_TESTS_ENABLED', 'false').lower() == 'true')


@pytest.mark.integration
@pytest.mark.skipif(not hardware_enabled(), 
...
class TestViamCompliance:
    ...
    @pytest.mark.asyncio
    async def test_sensor_interface_compliance(self, basic_config, mock_hx711_library, mock_gpio):
...
class TestRealWorldScenarios:
    ...
    @pytest.mark.asyncio
    async def test_configuration_changes(self, basic_config, mock_hx711_library, mock_gpio):
    ...
    @pytest.mark.asyncio
    async def test_stress_testing_multiple_operations(self, loadcell_sensor, mock_hx711_library):
